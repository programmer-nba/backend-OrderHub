...
getAll = async (req, res)=>{
    try{
        const findAll = await bankCode.find()
            if(!findAll){
                return res
                        .status(404)
...
createBank = async (req, res)=>{
    try{
        ...
        const findDuplicate = await bankCode.findOne(
            {
                aka:aka
            }
        )
        ...
        const create = await bankCode.create({
            aka:aka,
            name:name
        })
            if(!create){
...
getByAKA = async (req, res)=>{
    try{
        ...
        const findAka = await bankCode.findOne({aka:aka})
            if(!findAka){
                return res
                        .status(404)
...
delendByAKA = async (req, res)=>{
    try{
        ...
        const findId = await bankCode.findOneAndDelete({_id:id})
            if(!findId){
                return res
                        .status(404)
...
updateBank = async (req, res)=>{
    try{
        ...
        const aka = req.body.aka
        const name = req.body.name
        const findId = await bankCode.findOneAndUpdate(
            {_id:id},
            {
                aka:aka,
                name:name
